            ...
            if slot == 'booked':
                ...
                if bstate[domain]['book'][slot] != "":
                    ...
                    summary_bvalue.append(["{}-book {}".format(domain, slot.strip().lower()),
                                           normalize(bstate[domain]['book'][slot].strip().lower())])  # (["book", domain, slot, bstate[domain]['book'][slot]])
                else:
                    booking.append(0)
        if domain == 'train':
        ...
        for slot in bstate[domain]['semi']:
            ...
            if bstate[domain]['semi'][slot] == 'not mentioned':
                ...
                summary_bvalue.append(["{}-{}".format(domain, slot.strip().lower()),
                                       normalize(bstate[domain]['semi'][slot].strip().lower())])  # (["semi", domain, slot, bstate[domain]['semi'][slot]])
            if slot_enc != [0, 0, 0]:
                domain_active = True
